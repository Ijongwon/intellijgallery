...
const socket_func = function(){
    ...
    websocket.onopen = on_Open           //소켓 통신 시작(초기화)
    websocket.onmessage = on_Message     //소켓 통신 데이터 수신 이벤트. onmessage는 소켓 이벤트, 함수 이름 on_Message
...
//userid 세션의 값이 있을 때 즉 로그인이 됐을때 만 소켓통신 시작.
...
//좋아요 체크박스 이벤트 함수 : 이 함수는 어디서 호출되어야 할까요
function heart(idx,value,writer){
    const heartDto = {writer:writer,idx:idx,value:value,userid:userid}
    // 서비스 클래스에서 Heart 타입의 자바 객체로 매핑 될 것 이므로 key 이름이 Heart 클래스 필드와 일치해야 합니다.
