...
public class GalleryServieImpl implements GalleryService{
	...
	public int save(Gallery dto) {		//controller에서 넘어온 파라미터 값들로 insert + 파일업로드
		...
		//파일업로드
		for(MultipartFile f:dto.getPics()) {		//첨부파일이 여러개 일때 반복
...
//				String prefix = ofilename.substring(0, ofilename.lastIndexOf("."));   //오리지날 파일이름
	...
	public Map<String,String> processHeartCount(String data) {  //data는 json 문자열
		//이 메소드는 여러개의 insert,update,delete DML을 실행하므로 한 묶음의 실행인 트랜잭션 처리가 필요하여 애노테이션을 추가함.
		// 맨 위를 보세요.
		//좋아요 문자열 정보를 받아 객체로 만들고 다시 필요한 결과값을 직렬화 한다.
		Heart heart=null;
		Map<String,String> resultMap = new HashMap<>();
		try{
			ObjectMapper objMapper = new ObjectMapper();		//json과 자바객체 변환하는 매퍼
			heart = objMapper.readValue(data, Heart.class);		//받은 문자열 -> 역직렬화(Heart 타입 객체)
			log.info(">>>>> service heart 정보 : {}",heart);		// 누가 어느 글에 좋아요를 등록/취소 했는지에 대한 값은 heart객체에 저장
			...
			HeartResponseDTO responseDTO = HeartResponseDTO.builder().alarm(alarm).hearts(cnt).writer(heart.getWriter()).build();
			String result = objMapper.writeValueAsString(responseDTO);
