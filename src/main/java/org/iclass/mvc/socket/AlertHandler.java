...
import org.iclass.mvc.service.GalleryServieImpl;
...
// 텍스트 기반의 통신을 구현해 볼 것 이므로 'TextWebSocketHandler'를 상속받아서 작성한다.
	...
	//소켓 연결이 이루어 진 후에 할일 처리 - 새로운 클라이언트와의 연결이 발생하면 실행되는 메소드
		...
		// js(클라이언트)가 보내면 서버는 받은 데이터를 이 메소드가 처리합니다.
	   @Override
		protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
			...
			if(content.startsWith("open")) {			//2-e. 변수명을 이해하기 쉬운 이름으로 변경(받은 메시지 변수)
				...
				for(WebSocketSession ws : wsslist)		// 모든 접속자에게 보내기
					ws.sendMessage(new TextMessage(senderId +" 님이 접속하였습니다."));//메시지 보내기: 보낼 메시지 타입은 TextMessage
					// ws에게 메시지 보내기 : 보낼 메시지 타입은 TextMessage

				//map에 사용자 id 를 key , 소켓세션을 value로 저장하기 - 좋아용에서 필요
